...
namespace Andromeda.Network.Internal
{
    internal class DuplexPipeStream : Stream
    {
        ...
        [SuppressMessage("Reliability", "CA2012", Justification = "ValueTask uses .GetAwaiter().GetResult() if necessary")]
        ...
        {
            if (buffer is not null) _output.Write(new ReadOnlySpan<byte>(buffer, offset, count));
