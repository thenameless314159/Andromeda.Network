...
{
    public class PipeMessageEncoder<TMeta> : PipeFrameEncoder<TMeta>, IFrameMessageEncoder<TMeta>
        where TMeta : class, IFrameMetadata
    {
        public PipeMessageEncoder(PipeWriter pipe, IMetadataEncoder encoder, IMessageWriter serializer, SemaphoreSlim? singleWriter = default)
            : base(pipe, encoder, singleWriter) => _serializer = serializer;

        public PipeMessageEncoder(Stream stream, IMetadataEncoder encoder, IMessageWriter serializer, SemaphoreSlim? singleWriter = default)
